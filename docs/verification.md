...
Raw verification records are in [`artifacts/submission/final_recovery/verification/`](../artifacts/submission/final_recovery/verification/). The primary ROS CLI logs are in the adjacent `ros/` directory. The repository tracks the cleaned submission files and the reviewed extended-demo changes, excluding virtual environments, caches and unfinished local experiments. The extended-demo update is committed and pushed, so the Git history is the single source of record for every submitted file. Absolute development paths in the archived evaluation and log files were replaced with the `<repo>` and `<independent-checkout>` placeholders; only those path strings changed, and `manifest.json` records the resulting `reference/trajectory.json` checksum.

## Extended interactive demo

`RUN_DEMO.sh` continues physical control after the 15-second assessment. R restarts the same seeded recovery, and closing the viewer ends the process. `RUN_DEMO.sh --once` retains the bounded demo. Seven new lifecycle tests cover continued stepping, early close, restart, fault pausing/restart, bounded mode and invalid headless usage. A real viewer smoke check completed recovery, dispatched the R callback, completed recovery again and closed cleanly; its result is in `verification/extended_demo_window.json`. `scripts/verify_extended_hold.py` drives the same runtime headlessly for a fixed simulated duration, because the windowed demo cannot record how long the stance survives; its 120-second record is in `verification/extended_hold_120s.json`. The trained weights and the assessment/ROS time limits are unchanged.
