...
A bounded off-reference data aggregation experiment is being evaluated before any longer training. Labels remain the scripted teacher's time-based commands, not an optimal corrective expert. Any subsequent PPO stage must be evaluated as its own frozen actor, with crouch, standing and supine outcomes reported separately.
...
A separate PPO fine-tune is budgeted for 300 seconds, 256 environments, seed 4. It starts from the aggregated actor/normalizer and parent critic, with fresh optimizer, noise and counters. The actor normalizer stays frozen. Configuration: action std .015, bounds .005–.04, learning rate 1e-5, clip .05, no entropy bonus. The scripted teacher is absent during PPO rollouts and policy evaluation. The stage starts crouched, not supine. Evaluation of the saved final actor determines whether it replaces the supervised warm start.
