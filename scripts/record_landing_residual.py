"""Record the external teacher plus our trained bounded PPO ankle corrections."""
import argparse
import copy
import contextlib
import json
...
from train_landing_residual import Residual, episode, sha
...
class Recording:
    ...
    def __call__(self, info, data, step, hold, monitor):
        if self.renderer is None:
            ...
            self.writer = imageio.get_writer(self.output / 'teacher_plus_ppo_residual.mp4', fps=25)
            if self.live:
        ...
        if step % 2 == 1:
            ...
            draw.text((12, 7), 'EXTERNAL TEACHER + TRAINED PPO CORRECTION', fill=(255, 220, 95), font=font)
            draw.text((12, 29), 'Teacher supplies recovery | PPO adjusts ankle targets', fill='white', font=font)
            draw.text((12, 51), f'Time {data.time:5.2f} s | clean hold {hold:.2f} s | limits '
                      + ('PASS' if monitor.ok else 'FAIL'), fill='white', font=font)
            draw.text((12, 568), 'Bounded local PPO correction | no state teleporting', fill=(255, 220, 95), font=font)
        ...
        if self.viewer is not None and self.viewer.is_running():
            ...
            self.viewer.set_texts((mujoco.mjtFontScale.mjFONTSCALE_150, mujoco.mjtGridPos.mjGRID_TOPLEFT,
                                  'TEACHER + TRAINED PPO RESIDUAL\nExternal policy provides recovery\nSimulation time\nClean standing hold\nTrajectory limits',
...
def main():
    ...
    model = Residual().eval()
    model.load_state_dict(torch.load(a.checkpoint, weights_only=True, map_location='cpu')['model'])
    graph = VendorGraph(a.asset_dir)
    recording = Recording(a.output, a.live)
    try:
        result, _, _ = episode(a.seed, model, graph=graph, on_step=recording)
    finally:
        recording.close()
    ...
    keys = ['passed', 'terminal_hold', 'max_clean_hold', 'max_height', 'limits',
            'final_checks', 'final_stance', 'sim_time', 'max_residual_rad']
