...
def episode(info,name,start_mode,seed=9001,gravity_offsets=False):
 ...
 if gravity_offsets:
  plan=Path('artifacts/validation/floor_transition/kinematics/statics')/(name+'.json');payload=json.loads(plan.read_text());ik=np.array([payload['gravity_offset_targets']['target_rad'][n] for n in info.names])
  if np.any(ik<info.lower+.05) or np.any(ik>info.upper-.05):raise ValueError('Offset target outside guarded range')
