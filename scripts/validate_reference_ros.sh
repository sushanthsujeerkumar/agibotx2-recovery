#!/usr/bin/env bash
# Fresh ROS build + actual reference-controller success, busy and timeout checks.
set -euo pipefail
repo_dir="$(cd "$(dirname "$0")/.." && pwd)"
assets="${X2_VENDOR_ASSETS_DIR:-$repo_dir/.cache/vendor_reference}"
evidence="${X2_REFERENCE_ROS_EVIDENCE:-$repo_dir/artifacts/local_evaluation/reference_ros_$(date +%Y%m%d_%H%M%S)_$$}"
if [[ -e "$evidence" ]]; then
...
trap cleanup EXIT
setsid ros2 launch x2_recovery_ros recovery.launch.py controller:=reference_residual \
    checkpoint:="$repo_dir/artifacts/experiments/landing_residual_ppo/update_004.pt" \
    vendor_assets:="$assets" physics_profile:=guarded_v2 seed:=11301 render:=false \
