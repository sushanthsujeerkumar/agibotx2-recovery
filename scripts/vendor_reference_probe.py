...
def run(graph,mode,timescale,seed,finish='blend',on_step=None):
    ...
    config=yaml.safe_load((graph.directory/'ground_init_config.yaml').read_text());init=info.nominal.copy()
    ...
    for step in range(round(duration/CONTROL_DT)):
        ...
        if t<5:
            ...
            if mode=='reference':command[order]=reference[frame]
                ...
                obs=np.concatenate([reference[frame],velocity[frame]/timescale,rotation.T@np.array([0.,0.,-1.]),
        ...
        if on_step is not None:on_step(info,data,step,phase,hold,monitor)
    ...
    return {'scope':'External vendor compatibility diagnostic; not our PPO, not official runner reproduction',
            ...
            'observation_contract':'reference joint position29 + reference velocity29/timescale + pelvis gravity3 + pelvis local angular velocity3*0.25 + q29-default + dq29*0.05 + previous raw action29',
...
def main():
    ...
    graph=VendorGraph(args.asset_dir);result,states=run(graph,args.mode,args.timescale,args.seed,args.finish)
