#!/usr/bin/env python3
"""Capture a checked scripted deep-crouch-to-standing teacher, not RL recovery.

The teacher begins at reset_deep_crouch's physically attained state. No position or
...
from x2_recovery.physics import TrajectoryLimits, reset_deep_crouch
...
def deep_crouch_target(info):
    ...
    for side in ("left", "right"):
        for joint, value in (("hip_pitch", -.9), ("knee", 1.6), ("ankle_pitch", -.7)):
...
def episode(info, seed, duration, transition):
    ...
    if initial_checks["height"]:
        raise RuntimeError("Deep reset unexpectedly already satisfies standing height")
    monitor = TrajectoryLimits(info)
    monitor.observe(data)
    low = deep_crouch_target(info)
    if np.any(low < info.lower + .05) or np.any(low > info.upper - .05):
        raise RuntimeError("Deep teacher command exceeds the guarded target margin")
    ...
    min_foot_forces = last_forces[:2].copy()
    max_other_support = float(last_forces[2])
    max_target_clamp = 0.
    ...
    for step in range(round(duration / CONTROL_DT)):
        ...
        max_target_clamp = max(max_target_clamp, float(np.abs(target - np.clip(target, info.lower + .05, info.upper - .05)).max()))
        ...
        for _ in range(info.substeps):
            ...
            monitor.observe(data)
            last_forces = ground_forces(info, data)
            min_foot_forces = np.minimum(min_foot_forces, last_forces[:2])
            max_other_support = max(max_other_support, float(last_forces[2]))
    ...
    passed = complete and hold >= HOLD_SECONDS - 1e-8 and bool(clean) and monitor.ok and max_target_clamp <= 1e-10
    row = {
        ...
        "maximum_command_target_clamp_rad": max_target_clamp,
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("artifacts/validation/deep_crouch_stage/reference"))
    parser.add_argument("--seconds", type=float, default=10.)
    parser.add_argument("--transition-seconds", type=float, default=4.)
    parser.add_argument("--validation-first-seed", type=int, default=7001)
    parser.add_argument("--validation-episodes", type=int, default=5)
    parser.add_argument("--training-first-seed", type=int, default=301)
    ...
    if (args.output / "summary.json").exists() or (args.output / "training_pairs.npz").exists():
    ...
    config = {
        "task": "Scripted deep-crouch-to-standing teacher; NOT learned RL or supine recovery",
        ...
        "reset": "reset_deep_crouch: physically generated deep crouch, never state-clipped",
        "reset_time_excluded_from_episode_s": 5.4,
        "teacher_crouch_command_rad": {"hip_pitch": -.9, "knee": 1.6, "ankle_pitch": -.7},
        ...
        "pass_rule": "Complete configured duration, >=2 s terminal clean hold, no limit violations",
