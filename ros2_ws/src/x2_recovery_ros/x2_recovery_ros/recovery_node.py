...
class RecoveryNode(Node):
    ...
    def _start_callback(self, _request, response):
        ...
        if config['controller'] not in ('scripted', 'policy', 'full_recovery'):
            response.success = False
            response.message = 'controller must be scripted, policy or full_recovery'
            return response
        if config['physics_profile'] not in ('legacy', 'guarded_v2'):
            response.success = False
            response.message = 'physics_profile must be legacy or guarded_v2'
            return response
        if config['controller'] in ('policy', 'full_recovery') and not config['checkpoint']:
            response.success = False
