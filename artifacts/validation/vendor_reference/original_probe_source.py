...
def run(graph,mode,timescale,seed):
    ...
    end=5.+(len(reference)-1)*CONTROL_DT*timescale;duration=end+6.
    ...
    return {'scope':'External vendor compatibility diagnostic; not our PPO, not official runner reproduction',
            'mode':mode,'seed':seed,'reference_timescale':timescale,'planned_duration':duration,
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--mode',choices=['reference','vendor_actor'],required=True);p.add_argument('--timescale',type=float,default=1.);p.add_argument('--seed',type=int,default=9201);p.add_argument('--output',required=True);args=p.parse_args()
    out=Path(args.output);out.mkdir(parents=True,exist_ok=True)
    if (out/'result.json').exists():raise ValueError('Choose fresh output')
    graph=VendorGraph();result,states=run(graph,args.mode,args.timescale,args.seed)
